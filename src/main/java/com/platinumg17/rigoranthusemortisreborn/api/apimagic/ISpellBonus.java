...
public interface ISpellBonus {
    ...
    default AbstractAugment getBonusAugment(ItemStack stack){
        if(!stack.hasTag() || !stack.getTag().contains(AUGMENT))
            return null;
        return RigoranthusEmortisRebornAPI.getInstance().getSpell_map().get(stack.getTag().getString(AUGMENT)) != null ? (AbstractAugment)RigoranthusEmortisRebornAPI.getInstance().getSpell_map().get(stack.getTag().getString(AUGMENT)): null;
    }
    /**
