...
public class PsyglyphicBlock extends Block {

    public PsyglyphicBlock(Properties properties) {super(properties);}
    ...
    public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        ...
        if(ItemStack.isSame(itemstack, bookStack)) {
            if(!worldIn.isClientSide) {
                ...
                ItemEntity itementity = new ItemEntity(worldIn, (double) pos.getX() + 0.5D + (double) direction1.getStepX() * 0.65D, (double) pos.getY() + 0.1D, (double) pos.getZ() + 0.5D + (double) direction1.getStepZ() * 0.65D, new ItemStack(ItemInit.PSYGLYPHIC_SCRIPT.get(), amountInHandStack));
