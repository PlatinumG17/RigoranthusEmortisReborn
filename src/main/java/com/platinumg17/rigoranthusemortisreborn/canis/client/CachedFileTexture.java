...
public class CachedFileTexture extends SimpleTexture {
        ...
        Minecraft.getInstance().execute(() -> {
            if (!this.textureUploaded) {
                ...
                if (this.cacheFile.isFile() && this.cacheFile.exists()) {
                    RigoranthusEmortisReborn.LOGGER.debug("Loading canis texture from local cache ({})", this.cacheFile);
                ...
                if (nativeimage != null) {
                    ...
                    RigoranthusEmortisReborn.LOGGER.warn("Was unable to set image ({})", this.cacheFile);
                }
            }
        });
    }
    ...
    private NativeImage loadTexture(InputStream inputStreamIn) {
        ...
        try {
            ...
            RigoranthusEmortisReborn.LOGGER.warn("Error while loading the skin texture", ioexception);
        }
        return nativeimage;
    }
}
