...
public class WaywardTravellerScreen extends ContainerScreen<WaywardTravellerContainer> {
    ...
    public WaywardTravellerScreen(WaywardTravellerContainer waywardTraveller, PlayerInventory playerInventory, ITextComponent displayName) {
    ...
    protected void renderBg(MatrixStack stack, float partialTicks, int xMouse, int yMouse) {
        ...
        this.minecraft.getTextureManager().bind(Resources.GUI_WAYWARD_TRAVELLER);
        ...
        for (int j1 = 0; j1 < 3; j1++)
            for (int k1 = 0; k1 < MathHelper.clamp(this.getMenu().getCanisLevel(), 0, 5); k1++)
                this.blit(stack, l + 78 + 18 * k1, i1 + 9 + 18 * j1 + 15, 197, 2, 18, 18);

        InventoryScreen.renderEntityInInventory(l + 42, i1 + 51, 30, (float)(l + 51) - xMouse, (float)((i1 + 75) - 50) - yMouse, this.getMenu().getCanis());
    }
}
