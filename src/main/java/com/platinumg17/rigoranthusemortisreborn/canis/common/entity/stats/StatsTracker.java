package com.platinumg17.rigoranthusemortisreborn.canis.common.entity.stats;
...
public class StatsTracker {
    ...
    public void writeAdditional(CompoundNBT compound) {
        ...
        for (Entry<EntityType<?>, Integer> entry : this.ENTITY_KILLS.entrySet()) {
            ...
            NBTUtilities.putRegistryValue(stats, "type", entry.getKey());
    ...
    public void readAdditional(CompoundNBT compound) {
        ...
        for (int i = 0; i < killList.size(); i++) {
            ...
            EntityType<?> type = NBTUtilities.getRegistryValue(stats, "type", ForgeRegistries.ENTITIES);
    ...
    public int getKillCountFor(EntityType<?> type) {return this.ENTITY_KILLS.getOrDefault(type, 0);}
    ...
    public int getTotalKillCount() {return this.killCount.get();}
    public void incrementKillCount(Entity entity) {this.incrementKillCount(entity.getType());}
    private void incrementKillCount(EntityType<?> type) {this.ENTITY_KILLS.compute(type, (k, v) -> (v == null ? 0 : v) + 1);}
    public void increaseDamageDealt(float damage) {this.damageDealt += damage;}
    public void increaseDistanceOnWater(int distance) {this.distanceOnWater += distance;}
    public void increaseDistanceInWater(int distance) {this.distanceInWater += distance;}
    public void increaseDistanceSprint(int distance) {this.distanceSprinting += distance;}
    public void increaseDistanceSneaking(int distance) {this.distanceSneaking += distance;}
    public void increaseDistanceWalk(int distance) {this.distanceWalking += distance;}
    public void increaseDistanceRidden(int distance) {this.distanceRidden += distance;}
}