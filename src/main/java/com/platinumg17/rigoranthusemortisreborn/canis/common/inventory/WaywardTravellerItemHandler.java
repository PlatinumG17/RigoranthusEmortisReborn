...
public class WaywardTravellerItemHandler extends ItemStackHandler {

    public WaywardTravellerItemHandler() {
        super(15);
    }
    ...
    public void deserializeNBT(CompoundNBT compound) {
        if (compound.contains("items", Constants.NBT.TAG_LIST)) {
            ...
            this.onLoad();
        } else if (compound.contains("waywardtravelleritems", Constants.NBT.TAG_LIST)) {
            ListNBT tagList = compound.getList("waywardtravelleritems", Constants.NBT.TAG_COMPOUND);
            for (int i = 0; i < tagList.size(); i++) {
                CompoundNBT itemTag = tagList.getCompound(i);
                int slot = itemTag.getInt("Slot");
