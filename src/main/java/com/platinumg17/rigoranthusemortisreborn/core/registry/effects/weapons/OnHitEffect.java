...
public interface OnHitEffect {
    ...
    OnHitEffect PSYCHO_BLIGHT = (stack, target, attacker) -> {
        ...
        if(!attacker.level.isClientSide && attacker instanceof PlayerEntity && attacker.getRandom().nextFloat() < .1) {
            ...
            IFormattableTextComponent message = new TranslationTextComponent("message.psycho_blight." + key);
    ...
    OnHitEffect SWORD_DROP = (stack, target, attacker) -> {
        if(!attacker.getCommandSenderWorld().isClientSide && attacker.getRandom().nextFloat() < .25) {
            ...
            stack.shrink(1);
            attacker.sendMessage(new TranslationTextComponent(sord.getItem().getDescriptionId() + "." + SWORD_DROP_MESSAGE), Util.NIL_UUID);
        }
    };
    ...
    OnHitEffect SPACE_TELEPORT = onCrit((stack, target, attacker) -> {
        ...
        for(int i = 0; i < 16; ++i) {
            ...
            double newPosY = MathHelper.clamp(attacker.getY() + (double) (attacker.getRandom().nextInt(16) - 8), 0.0D, worldIn.getHeight() - 1);
...
//                ServerPlayerEntity.getData(serverPlayer);
//
...
//    }
    ...
    static OnHitEffect userPotionEffect(Supplier<EffectInstance> effect) {return (stack, target, attacker) -> attacker.addEffect(effect.get());}
    static OnHitEffect enemyPotionEffect(Supplier<EffectInstance> effect) {return (stack, target, attacker) -> target.addEffect(effect.get());}
