...
public class TameCanisAttackGoal extends Goal {

    protected final CanisEntity mob;
    ...
    public TameCanisAttackGoal(CanisEntity canis, boolean followUnseen) {
        this.mob = canis;
    ...
    public void tick() {
        ...
        if(BlockUtil.distanceFrom(this.mob.position, livingentity.position) <= 3){
            this.arrived = true;
            Networking.sendToNearby(mob.level, mob, new PacketAnimEntity(mob.getId(), CanisEntity.Animations.BITING.ordinal()));
        }
