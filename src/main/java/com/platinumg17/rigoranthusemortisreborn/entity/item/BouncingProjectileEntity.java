...
public class BouncingProjectileEntity extends ProjectileItemEntity {
    ...
    public BouncingProjectileEntity(EntityType<? extends BouncingProjectileEntity> type, World worldIn) {super(type, worldIn);}

    public BouncingProjectileEntity(EntityType<? extends BouncingProjectileEntity> type, double x, double y, double z, World worldIn) {super(type, x, y, z, worldIn);}
    ...
    protected void onHit(RayTraceResult result) {
        ...
        if(result.getType() == RayTraceResult.Type.ENTITY) {
            ...
            if(Block.canSupportCenter(level, blockPos, blockFace) && blockResult.isInside()) {++inBlockTicks;}
        }
        if(bounce > 10) {resetThrower();}
    }
    ...
    public void tick() {
        ...
        if(this.tickCount >= maxTick || inBlockTicks >= 2) {resetThrower();}
        super.tick();
    }
    ...
    protected Item getDefaultItem() {
        return ItemInit.RICOCHET_ROUND.get();
    }
