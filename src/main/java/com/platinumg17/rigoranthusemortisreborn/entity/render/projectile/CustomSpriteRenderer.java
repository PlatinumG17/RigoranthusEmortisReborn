package com.platinumg17.rigoranthusemortisreborn.entity.render.projectile;
...
public class CustomSpriteRenderer<T extends Entity & RendersAsItem> extends EntityRenderer<T> {
    ...
    private class FakeEntity extends Entity implements IRendersAsItem {
        private T entity;
        FakeEntity(EntityType<?> type) {super(type, null);}
        void setEntity(T entity) {
            this.entity = entity;
            this.setPos(entity.getX(), entity.getY(), entity.getZ());
        }
        @Override protected void defineSynchedData() {}
        @Override protected void readAdditionalSaveData(CompoundNBT compound) {}
        @Override protected void addAdditionalSaveData(CompoundNBT compound) {}
        @Override public IPacket<?> getAddEntityPacket() {return null;}
        @Override public ItemStack getItem() {return entity.getItem();}
        @Nullable @Override public Team getTeam() {return entity.getTeam();}
        @Override public boolean shouldShowName() {return entity.shouldShowName();}
        @Override public boolean hasCustomName() {return entity.hasCustomName();}
        @Override public ITextComponent getDisplayName() {return entity.getDisplayName();}
        @Override public Pose getPose() {return entity.getPose();}
    }

    private class ModifiedSpriteRenderer extends SpriteRenderer<FakeEntity> {
        ModifiedSpriteRenderer(EntityRendererManager renderManagerIn, ItemRenderer itemRendererIn) {super(renderManagerIn, itemRendererIn);}
        ResourceLocation getTexture(FakeEntity entity) {return getTextureLocation(entity);}
    }
}