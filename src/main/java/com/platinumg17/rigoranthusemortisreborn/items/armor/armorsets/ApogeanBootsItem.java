...
import com.platinumg17.rigoranthusemortisreborn.magica.common.network.PacketApogeanDoubleJump;
...
public class ApogeanBootsItem extends ApogeanArmor {
    public ApogeanBootsItem(EquipmentSlotType slot) {
        super(slot);
    ...
    public boolean isEquippedBy(@Nullable LivingEntity entity) {
        if (entity instanceof PlayerEntity && Config.enableArmorSetBonuses.get()) {
            ...
            return boots.getItem() == Registration.APOGEAN_N_BOOTS.get() && legs.getItem() == Registration.APOGEAN_N_LEGS && chest.getItem() == Registration.APOGEAN_N_CHEST && helm.getItem() == Registration.APOGEAN_N_HELMET;
        }
        else return false;
    }
                ...
                if (event.phase == TickEvent.Phase.END && player != null && player.input != null) {
                    if ((player.isOnGround() || player.onClimbable()) && !player.isInWater()) {
                        ...
                        if (isEquippedBy(player)) {
                            Networking.INSTANCE.sendToServer(new PacketApogeanDoubleJump());
                            jump(player);
                        }
                    }
                }
