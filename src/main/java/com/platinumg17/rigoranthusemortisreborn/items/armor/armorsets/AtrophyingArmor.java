...
public class AtrophyingArmor extends ArmorItem {
    ...
    public AtrophyingArmor(IArmorMaterial materialIn, EquipmentSlotType slot, Properties builder) {
    ...
    public void onArmorTick(ItemStack itemStack, World world, PlayerEntity player) {
        if (Config.enableArmorSetBonuses.get()) {
            ...
            if (boots.getItem() == Registration.ATROPHYING_NETHERITE_BOOTS && legs.getItem() == Registration.ATROPHYING_NETHERITE_LEGGINGS && chest.getItem() == Registration.ATROPHYING_NETHERITE_CHESTPLATE && helm.getItem() == Registration.ATROPHYING_NETHERITE_HELMET) {
                player.addEffect(new EffectInstance(RigoranthusEffectRegistry.ATROPHYING_SET_BONUS, 5, 1));
                this.previousEquip = true;
            } else if (this.previousEquip) {
                player.removeEffect(RigoranthusEffectRegistry.ATROPHYING_SET_BONUS);
