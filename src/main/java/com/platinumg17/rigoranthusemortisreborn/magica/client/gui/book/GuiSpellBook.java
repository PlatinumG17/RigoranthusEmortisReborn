...
import com.platinumg17.rigoranthusemortisreborn.magica.client.gui.buttons.*;
...
public class GuiSpellBook extends BaseBook {
    ...
    public GuiSpellBook(RigoranthusEmortisRebornAPI api, CompoundNBT tag, int tier, String unlockedSpells) {
        super();
        this.api = api;
    ...
    public void init() {
        ...
        spell_name = new NoShadowTextField(minecraft.font, bookLeft + 32, bookTop + FULL_HEIGHT - 11,
                88, 12, null, new TranslationTextComponent("rigoranthusemortisreborn.spell_book_gui.spell_name"));
        ...
        searchBar = new NoShadowTextField(minecraft.font, bookRight - 73, bookTop +2,
                54, 12, null, new TranslationTextComponent("rigoranthusemortisreborn.spell_book_gui.search"));
        ...
        if(spell_name.getValue().isEmpty())
            spell_name.setSuggestion(new TranslationTextComponent("rigoranthusemortisreborn.spell_book_gui.spell_name").getString());

        if(searchBar.getValue().isEmpty())
            searchBar.setSuggestion(new TranslationTextComponent("rigoranthusemortisreborn.spell_book_gui.search").getString());
...
//
        ...
        addButton(new GuiImageButton(bookLeft - 15, bookTop + 46, 0, 0, 23, 20, 23,20, "textures/gui/color_wheel_bookmark.png",this::onColorClick)
                .withTooltip(this, new TranslationTextComponent("rigoranthusemortisreborn.gui.color")));
...
//        PatchouliAPI.get().openBookGUI(Registry.ITEM.getKey(MagicItemsRegistry.emorticOrigins));
    ...
    public void onFamiliarClick(Button button){
        ...
        IFamiliarCap cap = FamiliarCap.getFamiliarCap(RigoranthusEmortisReborn.proxy.getPlayer()).orElse(null);
        if(cap != null){
            familiarHolders = cap.getUnlockedFamiliars();
        }
        Collection<String> finalFamiliarHolders = familiarHolders;
        Minecraft.getInstance().setScreen(new GuiFamiliarScreen(api, RigoranthusEmortisRebornAPI.getInstance().getFamiliarHolderMap().values().stream().filter(f -> finalFamiliarHolders.contains(f.id)).collect(Collectors.toList())));
    }
    ...
    public static void open(RigoranthusEmortisRebornAPI api, CompoundNBT spell_book_tag, int tier, String unlockedSpells){
    ...
    public void drawBackgroundElements(MatrixStack stack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (validationErrors.isEmpty()) {
            ...
            ITextComponent textComponent = new TranslationTextComponent("rigoranthusemortisreborn.spell_book_gui.create")
        ...
        minecraft.font.draw(stack,new TranslationTextComponent("rigoranthusemortisreborn.spell_book_gui.clear").getString(), 177, 183, -8355712);
    }
    ...
    public void render(MatrixStack ms, int mouseX, int mouseY, float partialTicks) {
        ...
        spell_name.setSuggestion(spell_name.getValue().isEmpty() ? new TranslationTextComponent("rigoranthusemortisreborn.spell_book_gui.spell_name").getString() : "");
    }
}