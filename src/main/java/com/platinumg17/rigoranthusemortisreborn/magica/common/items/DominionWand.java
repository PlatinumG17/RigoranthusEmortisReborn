...
public class DominionWand extends ModItem{
    ...
    public ActionResultType interactLivingEntity(ItemStack doNotUseStack, PlayerEntity playerEntity, LivingEntity target, Hand hand) {
        ...
        if((getPos(stack) == null || getPos(stack).equals(new BlockPos(0,0,0))) && getEntityID(stack) == -1){
            ...
            PortUtil.sendMessage(playerEntity, new TranslationTextComponent("rigoranthusemortisreborn.dominion_wand.stored_entity"));
            return  ActionResultType.SUCCESS;
        }
    ...
    public ActionResultType useOn(ItemUseContext context) {
        ...
        if(getEntityID(stack) == - 1 && (getPos(stack) == null || getPos(stack).equals(new BlockPos(0,0,0)))){
            setPosTag(stack, pos);
            PortUtil.sendMessage(playerEntity, new TranslationTextComponent("rigoranthusemortisreborn.dominion_wand.position_set"));
            return ActionResultType.SUCCESS;
        }
    ...
    public void appendHoverText(ItemStack stack, @Nullable World world, List<ITextComponent> tooltip, ITooltipFlag p_77624_4_) {
        ...
        tooltip.add(getEntityID(stack) == -1 ?
                new TranslationTextComponent("rigoranthusemortisreborn.dominion_wand.no_entity") :
                new TranslationTextComponent("rigoranthusemortisreborn.dominion_wand.entity_stored"));
        if(pos == null){
            tooltip.add(new TranslationTextComponent("rigoranthusemortisreborn.dominion_wand.no_location"));
            return;
        }

        tooltip.add(new TranslationTextComponent("rigoranthusemortisreborn.dominion_wand.position_stored", getPosString(pos)));
    }

    public static String getPosString(BlockPos pos){
        return new TranslationTextComponent("rigoranthusemortisreborn.position", pos.getX(), pos.getY(), pos.getZ()).getString();
    }
}