...
//                this.path = null;
//                this.path = this.getPathToPos(this.targetPos, this.checkpointRange);
...
//            Vector3d target = this.path.getVectorFromIndex(this.advancedPathFindingEntity, Math.min(this.path.getpathLength() - 1, this.path.getpathIndex() + 0));
...
//
//            AxisAlignedBB checkBox = this.advancedPathFindingEntity.getBoundingBox().inflate(Math.signum(diff.x) * 0.2D, Math.signum(diff.y) * 0.2D, Math.signum(diff.z) * 0.2D);
//
...
//                        VoxelShape collisionShape = state.getShape(this.advancedPathFindingEntity.level, pos, ISelectionContext.of(this.advancedPathFindingEntity)).withOffset(pos.getX(), pos.getY(), pos.getZ());
//
