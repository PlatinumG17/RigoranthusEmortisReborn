...
public class EmortisMobSpawns {
    ...
    public static void onBiomeLoad(BiomeLoadingEvent event) {
        ...
        if (event.getCategory() == Biome.Category.THEEND || event.getCategory().equals(Biome.Category.MUSHROOM) || event.getCategory().equals(Biome.Category.NONE))
            return;
        ...
        List<DimensionType> dims = Arrays.asList();

        if (Config.sunderedCadaverSpawnWeight.get() > 0) {
            if (event.getCategory().equals(Biome.Category.NETHER) && Config.allowCadaverNetherSpawns.get()) {
        ...
        if (Config.necrawFasciiSpawnWeight.get() > 0) {
            if (event.getCategory().equals(Biome.Category.NETHER) && Config.allowNecrawNetherSpawns.get()) {
