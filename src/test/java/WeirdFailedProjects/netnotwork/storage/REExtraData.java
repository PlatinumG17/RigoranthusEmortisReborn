 ...
 * @author kirderf1
 */
public class REExtraData extends WorldSavedData {
    private static final Logger LOGGER = LogManager.getLogger();
    private static final String DATA_NAME = EmortisConstants.MOD_ID+"_extra";
    ...
    private REExtraData() {super(DATA_NAME);}
    ...
    public CompoundNBT save(CompoundNBT compound) {
        ...
        editRecoveryList.addAll(editPlayerRecovery.entrySet().stream().map(REExtraData::writeRecovery).collect(Collectors.toList()));
        editRecoveryList.addAll(activeEditData.stream().map(REExtraData::writeRecovery).collect(Collectors.toList()));
    ...
    public static REExtraData get(World world) {
    ...
    public static REExtraData get(MinecraftServer mcServer) {
        ServerWorld world = mcServer.getLevel(World.OVERWORLD);
        DimensionSavedDataManager storage = world.getDataStorage();
        REExtraData instance = storage.get(REExtraData::new, DATA_NAME);

        if(instance == null) {
            instance = new REExtraData();
    ...
    public void forEach(Consumer<EditData> consumer) {activeEditData.forEach(consumer);}
