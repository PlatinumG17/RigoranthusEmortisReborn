...
import static aspects.EnumAspect.HOPE;
...
public final class Title {
    ...
    public EnumClass getHeroClass() {return this.heroClass;}

    public EnumAspect getHeroAspect() {return this.heroAspect;}

    public void handleAspectEffects(ServerPlayerEntity player) {
//        if(!Config.enableAspectEffects.get())
//            return;
        PlayerData data = PlayerSavedData.getData(player);
        if(data.effectToggle()) {
            int level = data.getAspectProgressBar().getAspectLevel();
            EnumAspect aspect = data.getTitle().getHeroAspect();
            int potionLevel = (int) (aspectStrength[aspect.ordinal()] * level); //Blood, Breath, Doom, Heart, Hope, Life, Light, Mind, Rage, Space, Time, Void

            if(player.getCommandSenderWorld().getGameTime() % 380 == 0) {
                if(level > 18 && aspect == HOPE) {
    ...
    @Override public String toString() {return heroClass.toString() + " of " + heroAspect.toString();}

    public ITextComponent asTextComponent() {return new TranslationTextComponent(FORMAT, heroClass.asTextComponent(), heroAspect.asTextComponent());}
    ...
    @Override public int hashCode() {return Objects.hash(heroClass, heroAspect);}

    private static String makeNBTPrefix(String prefix) {return prefix != null && !prefix.isEmpty() ? prefix + "_" : "";}
